...
import cn.my.rxjavatest.operator.*;
...
public class OperatorActivity extends AppCompatActivity {
    ...
    private List<String> getTitles() {
        ...
        titles.add("RxJavaAct25_PublishSubject");
        titles.add("RxJavaAct26_AsyncSubject");
        titles.add("RxJavaAct27_BehaviorSubject");
        titles.add("RxJavaAct28_Completable");
        titles.add("RxJavaAct29_Flowable");
...
//        onNext() 会通知每个观察者，仅此而已
        listeners.add(RxJavaAct25_PublishSubject.class);
//        在调用 onComplete() 之前，除了 subscribe() 其它的操作都会被缓存，
//        在调用 onComplete() 之后只有最后一个 onNext() 会生效
        listeners.add(RxJavaAct26_AsyncSubject.class);
//        BehaviorSubject 的最后一次 onNext() 操作会被缓存，
//        然后在 subscribe() 后立刻推给新注册的 Observer
        listeners.add(RxJavaAct27_BehaviorSubject.class);
//        只关心结果，也就是说 Completable 是没有 onNext 的，
//        要么成功要么出错，不关心过程，在 subscribe 后的某个时间点返回结果
        listeners.add(RxJavaAct28_Completable.class);
//        专用于解决被压问题
        listeners.add(RxJavaAct29_Flowable.class);
